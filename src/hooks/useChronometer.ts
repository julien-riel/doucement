/**
 * Hook useChronometer - Logique commune pour chronomètre et minuterie
 *
 * Gère le décompte du temps avec persistance localStorage.
 * Utilisé comme base par useStopwatch (temps écoulé) et useTimer (compte à rebours).
 */
...
export interface UseChronometerOptions {
  /** ID de l'habitude */
  habitId: string
...
export interface UseChronometerReturn {
...
/**
 * Hook de base pour gérer un chronomètre avec persistance localStorage.
 * Fournit start, pause, stop, reset et le temps écoulé.
 */
export function useChronometer({
  ...
  initialValue = 0,
  onStop,
}: UseChronometerOptions): UseChronometerReturn {
  // État local
