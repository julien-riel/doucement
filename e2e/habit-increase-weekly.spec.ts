...
/**
 * Create test data for weekly habit with entries this week
 */
function createIncreaseWeeklyData() {
  resetCounters()
  const monday = getMondayOfCurrentWeek()

  const habit = createWeeklyHabit({
  ...
  // Create 3 entries for this week (Monday, Tuesday, Wednesday)
  const entries = [0, 1, 2].map((offset, index) => {
    const dateStr = addDays(monday, offset)
    return createEntry({
      id: `e${index + 1}`,
  ...
  return createAppData({
    ...
    preferences: {
      ...
      lastWeeklyReviewDate: getDaysAgo(7),
      notifications: {
        enabled: false,
...
/**
 * Create test data with 0 days this week
 */
function createIncreaseWeeklyEmptyData() {
...
test.describe('Habitude increase weekly', () => {
  test('affiche le compteur X/Y cette semaine', async ({ page }) => {
    ...
    await expect(
      habitCard.getByRole('heading', { name: 'Se coucher à heure fixe' })
    ).toBeVisible()
